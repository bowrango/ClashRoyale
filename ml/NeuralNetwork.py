...
# adjacency matrix --> data model --> system --> prediction
...
#
# Shorthand: "pd_" as a variable prefix means "partial derivative" "d_" as a variable prefix means "derivative"
# "_wrt_" is shorthand for "with respect to" "w_ho" and "w_ih" are the index of weights from hidden to output layer
...
class Neuron:
    ...
    #
    # The Wikipedia article on backpropagation [1] simplifies to the following, but most other learning material does
    # not [2] = actual output - target output
    #
    ...
    # The total net input into the neuron is squashed using logistic function to calculate the neuron's output: yⱼ =
    # φ = 1 / (1 + e^(-zⱼ)) Note that where ⱼ represents the output of the neurons in whatever layer we're looking at
    ...
    # constant) then is: = ∂zⱼ/∂wᵢ = some constant + 1 * xᵢw₁^(1-0) + some constant ... = xᵢ
